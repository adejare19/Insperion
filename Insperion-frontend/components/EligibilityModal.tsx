// Insperion-frontend/components/EligibilityModal.tsx
...
export function EligibilityModal({ isOpen, onClose, provider, userAddress }: EligibilityModalProps) {
  const [isChecking, setIsChecking] = useState(false);
  const [result, setResult] = useState<'eligible' | 'not-eligible' | null>(null);

  // --- NEW FHE ELIGIBILITY CHECK LOGIC ---
...
    try {
      // Get the contract instance (read-only view call doesn't need signer, but standardizes access)
      const registry = getRegistryContract(provider);

      // The FHEVM call: isEligible runs the check privately on the encrypted score
      const isUserEligible: boolean = await registry.isEligible(userAddress);
      
      setResult(isUserEligible ? 'eligible' : 'not-eligible');
      
    } catch (error) {
      console.error("FHE Eligibility check failed:", error);
      setResult('not-eligible');
    } finally {
      setIsChecking(false);
    }
...
  useEffect(() => {
    if (isOpen) {
      checkEligibility(); // Call the real check when the modal opens
    }
...
    // No changes needed in the JSX, as isChecking/result state still handles the UI
    <AnimatePresence>
      {isOpen && (
